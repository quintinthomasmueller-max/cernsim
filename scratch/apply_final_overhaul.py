...
if start_pos != -1:
    ...
    # Find the closing triple quotes after start_pos. In create_notebook.py, this ends the raw string.
    end_pos = text.find(end_marker, start_pos + len(start_marker))
    if end_pos != -1:
        print("Found html variable end!")
        
        # We need to construct the full replacement which includes the overhauled HTML
        # AND the javascript script tag and contents because they were originally in the raw string!
        # Let's read the javascript code of the original create_notebook.py to make sure we keep it.
        # Original script tag starts with `<script>` and ends before `"""`
        script_start = text.find("<script>", start_pos)
        if script_start != -1 and script_start < end_pos:
            print("Found <script> tag in original file!")
            original_js = text[script_start:end_pos]
            
            # Now let's perform replacements in the JS in-memory before writing back
            ...
            if dom_target in original_js:
                original_js = original_js.replace(dom_target, dom_replacement)
                print("Injected DOM references in JS!")
            else:
                print("Error: Could not find dom_target in JS!")
                exit(1)
                
            # Inject listeners
            listeners_target = 'btnAuto.addEventListener("click",async()=>{'
            listeners_replacement = """// GEO OVERLAY TOGGLE
let geoVisible = true;
...
btnAuto.addEventListener("click",async()=>{"""
            if listeners_target in original_js:
                original_js = original_js.replace(listeners_target, listeners_replacement)
                print("Injected click listeners in JS!")
            else:
                print("Error: Could not find listeners_target in JS!")
                exit(1)
                
            # Now build the full new variable contents
            new_html_content = overhauled_html + "\n\n" + original_js
            
            # Replace the html raw string content in the notebook generator
            new_text = text[:start_pos + len(start_marker)] + new_html_content + text[end_pos:]
            
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(new_text)
            print("Successfully overhauled create_notebook.py and saved!")
        else:
